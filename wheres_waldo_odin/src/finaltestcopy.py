            ...
            if queryObjResponse or linkedToQuoteResponse:
                for mapping in mappings:
                    ...
                    if customObject:
                        ...
                        try:
                            name = self.Quote.GetCustomField('CF_Sales_Person').Value
                        ...
                        ############Start Quoting Country Condition###############
                        try:
                            quotingCountry = self.Quote.GetCustomField('CF_QuotingCountry')
                            laborType = self.Quote.GetCustomField('CF_Labor_Type')
                            doaLaborType = self.Quote.GetCustomField('NAM_CF_Labor_Type')
                            contractEntity = self.Quote.GetCustomField('CF_NAM_Contractual_Entity')
                            if quotingCountry.Value.lower() == 'us':
                                laborType.Value = 'Non-Union'
                                doaLaborType.Value = 'Non-Union'
                                contractEntity.Value = '1090 - nVent Thermal LLC'
                                self.Quote.GetCustomField('CF_Tax_Classification').Value = ''
                                self.Quote.GetCustomField('CF_Tax_Options').Value = ''
                                self.Quote.GetCustomField('NAM_CF_Tax_Classification').Value = ''
                                self.Quote.GetCustomField('NAM_CF_Tax_Options').Value = ''
                            elif quotingCountry.Value.lower() == 'ca':
                                laborType.Value = 'Union'
                                doaLaborType.Value = 'Union'
                                contractEntity.Value = '8550 - nVent Thermal Canada Ltd'
                                self.Quote.GetCustomField('CF_Tax_Classification').Value = 'N/A'
                                self.Quote.GetCustomField('CF_Tax_Options').Value = 'N/A'
                                self.Quote.GetCustomField('NAM_CF_Tax_Classification').Value = 'N/A'
                                self.Quote.GetCustomField('NAM_CF_Tax_Options').Value = 'N/A'
                            taxClass = self.Quote.GetCustomField('CF_Tax_Classification').Value
                            Log.Info('taxClass ' + str(taxClass))
                        except:
                            Log.Info('quoting country condition failed')

                       		##################################End quote country condition#########################################

                        	################################  Start Sales Team Condition    ############################################
                            '''try:
                                sales_team = self.Quote.GetCustomField('CF_Profit_Center')
                                quotingCountry = self.Quote.GetCustomField('CF_QuotingCountry')
                                if sales_team == 'IHS':
                                    if quotingCountry.lower() == 'us':
                                        self.Quote['CF_Execution Team'] = 'Houston'
                                        self.Quote['NAM_CF_Execution Team'] = 'Houston'
                                    elif quotingCountry.lower() == 'ca':
                                        self.Quote['CF_Execution Team'] = 'Edmonton'
                                        self.Quote['NAM_CF_Execution Team'] = 'Edmonton'
                                elif sales_team == 'BIS':
                                    self.Quote['CF_Execution Team'] = 'Toronto (BIS)'
                                    self.Quote['NAM_CF_Execution Team'] = 'Toronto (BIS)'
                                execTeam = self.Quote['CF_Execution Team']
                                namExecTeam = self.Quote['NAM_CF_Execution Team']
                                businessArea = self.Quote['CF_Business Area_NAM']
                                taxClass = self.Quote['CF_Tax_Classification']
                                Log.Info('execteam ' + str(execTeam) + 'quotingCountry ' + str(quotingCountry) + ' sales team ' + str(sales_team) + 'business area ' + str(businessArea) + ' tax class ' + str(taxClass) )
                            except:
                            	Log.Info('sales team condition failed')'''

                         	################################  End Sales Team Condition##########################################
